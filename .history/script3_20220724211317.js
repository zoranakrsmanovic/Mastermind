...
let maxCellLen = 24;
let rowCellLen = 4;
let secret = ["background-angular", "background-javascript", "background-javascript","background-react"];
let secretMap = {};
let guess = [];
...
const displayColorCells = function(){
  ...
  for(let i = 0; i < result.red; i++){
    colorRow[currentRow].children[result.green + i].style.backgroundColor = 'red';
  }
  for(let i = 0; i < result.white; i++){
    colorRow[currentRow].children[result.red + i].style.backgroundColor = 'black';
  }
}
...
const checkAnswer = function () {
  ...
  // popunjavanje tajne kombinacije
