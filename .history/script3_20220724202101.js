...
const coloringCells = function(){ 
 if(currentRow == 0){
  currentColorCell = (4 + 4*0) - 1;
 }
 if(currentRow == 1){
  currentColorCell = (4 + 4*1) - 1;
 }
 if(currentRow == 2){
  currentColorCell = (4 + 4*2) - 1;
 }
 if(currentRow == 3){
  currentColorCell = (4 + 4*3) - 1;
 }
 if(currentRow ==4){
  currentColorCell = (4 + 4*4) - 1;
 }
 if(currentRow == 5){
  currentColorCell = (4 + 4*5) - 1;
 }
 for (let i = 0; i < currentColorCell.length; i++) {
    for(let j = 0; j < result.green; i++){
      colorCells[i].style.backgroundColor = "green";
    }
}
