  ...
  for (let i = 0; i < pickerBtn.length; i++) {
    ...
    pickerBtn[i].addEventListener("click", function (e) {
      ...
      if (elClass.includes(ANGULAR)) {
        choosenTechnology = ANGULAR;
      } else if (elClass.includes(REACT)) {
        choosenTechnology = REACT;
      } else if (elClass.includes(C_S_S)) {
        choosenTechnology = C_S_S;
      } else if (elClass.includes(JAVA_SCRIPT)) {
        choosenTechnology = JAVA_SCRIPT;
      }
...
// BRISANJE ODABIRA -- proveriti jos da li stvarno radi kako treba i ne remeti proveru
const deleteChoosenTechnology = function(){
  document.addEventListener('keydown', function(e){ //kada se event desi mozemo imati pristup objektu eventa
    ...
    if(e.key === 'Backspace'){
      ...
      currentCell--;
      if(currentCell == -1 || currentCell == 3 || currentCell == 7 || currentCell == 11 || currentCell == 15 || currentCell == 19){ //srediti u dinamicke vrednosti
        currentRow--;
      }
    }
    });
